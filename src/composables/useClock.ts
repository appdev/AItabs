...
export function useClock() {
  ...
  function update() {
    ...
    dateStr.value = now.format('M月D日')
    weekStr.value = '星期' + WEEK_NAMES[now.day()]
    fullDateStr.value = now.format('YYYY-MM-DD')

    const lunar = solarToLunar(now.year(), now.month() + 1, now.date())
    lunarStr.value = lunar.monthStr + lunar.dayStr
  }
